...
test('Select Economy type', async t => {
    await t.click(dashboardPage.parkingLot)
    await t.click(dashboardPage.typeSelect.withText('Economy Parking'))
    await t.expect(dashboardPage.parkingLot.value).eql('Economy')
    await t.wait(2000)
    ...
    await t.typeText(dashboardPage.entryTime,"7:00")
    await t.wait(2000)
    ...
    await t.wait(2000)
    await t.expect((dashboardPage.totalCost).withText('$ 9.00').exists).ok()
});